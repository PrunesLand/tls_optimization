...
[`differential_evolution_cluster_v3.py`](differential_evolution_cluster_v3.py),
backed by the tree-walk in
[`node_finder_v3.py`](node_finder_v3.py).
...
([`differential_evolution_cluster_v3.py:155`](differential_evolution_cluster_v3.py#L155)):
...
`LinkageTree.from_distance_json` (in [`node_finder.py`](node_finder.py), inherited
by the v3 subclass).
...
([`differential_evolution_cluster_v3.py:57`](differential_evolution_cluster_v3.py#L57)).
...
`find_node_decomposition` ([`node_finder_v3.py`](node_finder_v3.py)) returns a flat
...
for tls_id in members:
    if tls_id in _tls_to_genes:        # line 84
...
```python
trial = current_vect.clone()           # line 68 — start as a full copy of the parent
...
idx = torch.tensor(gene_idxs, dtype=torch.long, device=trial.device)
trial[i].index_copy_(0, idx, mutation_vector[i].index_select(0, idx))   # line 92
```
...
_last_target_sizes = targets.tolist()            # line 94
_last_actual_sizes = actual.tolist()             # line 95 (actual = kept counts)
```

These module-level captures let the generation loop print and store, per
individual, the `target → actual` cluster sizes
([`differential_evolution_cluster_v3.py:232`](differential_evolution_cluster_v3.py#L232)).
