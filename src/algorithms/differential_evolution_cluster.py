"""
Differential Evolution (SHADE) with linkage-tree cluster crossover.

Replaces SHADE's standard DE binomial crossover with a structured,
cluster-based variant driven by the Ward linkage tree over TLS distance
metrics.  For each individual *i*:

    target_size_i = round( CR_i * num_tls )

is looked up in the linkage tree via
`LinkageTree.find_node_closest_to_size`.  The trial vector copies the
genes belonging to TLS in that node from the mutant vector, and inherits
the remaining genes from the current (parent) vector.

This preserves SHADE's intent ("higher CR → more aggressive crossover")
but operates at the TLS-cluster granularity instead of per-gene Bernoulli
draws, so trial vectors are built from coherent intersection-groups
rather than scattered single genes.
...
Usage:  python -m src.algorithms.differential_evolution_cluster
"""
...
def run_single_de(baseline_data, num_genes, tls_to_genes,
                  ...
                  tree_name, dist_path):
    """Run one SHADE experiment whose crossover is the cluster variant."""
    ...
    print(f"\n{'='*60}")
    print(f"SHADE (EvoX) | Cluster crossover | Tree: {tree_name} | "
    ...
    results = {
        ...
        "algorithm": "shade_evox_cluster_crossover",
        "strategy": "random",
    ...
    out_file = out_dir / f"differential_evolution_cluster_{tree_name}.json"
