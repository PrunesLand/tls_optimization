"""
Differential Evolution (SHADE) with linkage-tree cluster crossover — v2.

Same SHADE-cluster-crossover scheme as
`src.algorithms.differential_evolution_cluster`, except the per-individual
target cluster size is computed against a *diversity-favouring* multiplier
rather than the full TLS count:

    target_size_i = round( CR_i * cr_multiplier )

`cr_multiplier` is derived per linkage tree by inspecting its node-size
histogram.  Walking from the largest size downwards, we record every
size whose node count is 1 and stop at the first size whose count is
greater than 1.  The last singleton size before that boundary is the
multiplier.

Example — if sizes (desc) are {36:1, 25:1, 23:1, 22:1, 18:2, 17:2,
15:1, ...}, the multiplier is 22 because 22 is the last singleton
before the first multi-count size (18).

Rationale: lower cluster sizes occur in more nodes (more diversity), so
capping the CR-scale at the singleton-boundary biases crossover toward
the high-diversity portion of the tree.

Runs 3 experiments, one per Ward distance tree (shortest / euclidian /
fastest); each uses its own linkage tree and its own derived multiplier.

Usage:  python -m src.algorithms.differential_evolution_cluster_v2
"""
...
_linkage_tree   = None  # LinkageTree for the active experiment
_tls_to_genes   = None  # dict[tls_id] -> (start, end) gene slice
_num_tls        = None  # genotype length at the TLS level (36 here)
_cr_multiplier  = None  # v2: scale applied to CR before rounding
_xover_rng      = None  # rng for tiebreaks in find_node_closest_to_size
...
def _cluster_binary_crossover(mutation_vector, current_vect, CR_vect):
    """SHADE-compatible crossover that operates on TLS clusters.
...
def compute_cr_multiplier(linkage_tree):
    """v2 multiplier: the last singleton size walking down from the root
    before the first size whose node count is greater than 1.

    Walks sorted-descending unique sizes; updates the running multiplier
    each time it sees a singleton, and stops at the first multi-count
    size.  If every size has count > 1 (shouldn't happen for a real
    Ward tree, since the root is always unique), falls back to the
    largest size.
    """
    counts = collections.Counter(linkage_tree.node_sizes.values())
    multiplier = max(counts)  # safe fallback (= num_tls = root size)
...
def run_single_de(baseline_data, num_genes, tls_to_genes,
                  ...
                  tree_name, dist_path):
    """Run one SHADE experiment whose crossover is the v2 cluster variant."""
    global _wrapper, _num_evals
    global _linkage_tree, _tls_to_genes, _num_tls, _cr_multiplier, _xover_rng
    ...
    _cr_multiplier = compute_cr_multiplier(_linkage_tree)
    _xover_rng     = rng

    print(f"\n{'='*60}")
    print(f"SHADE (EvoX) | Cluster crossover v2 | Tree: {tree_name} | "
          f"num_tls={_num_tls} | cr_multiplier={_cr_multiplier} | "
          f"Pop: {PYGAD_POPULATION_SIZE}")
    print(f"{'='*60}")
    ...
    results = {
        ...
        "algorithm": "shade_evox_cluster_crossover_v2",
        "strategy": "random",
        "pop_size": PYGAD_POPULATION_SIZE,
        "integrality": True,
        "num_genes": num_genes,
        "num_tls": _num_tls,
        "cr_multiplier": _cr_multiplier,
    ...
    out_file = out_dir / f"differential_evolution_cluster_v2_{tree_name}.json"
