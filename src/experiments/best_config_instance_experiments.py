...
REPETITIONS
-----------
Every configuration is stochastic, so each is run THREE times with distinct
seeds (rep 1 → 42, rep 2 → 43, rep 3 → 44, matching the seeds used to discover
these winners) and the three final ``best_fitness`` values are averaged.

OUTPUT (per instance, in src/outputs/instances/<instance>/)
-----------------------------------------------------------
* ``best_config_experiments.json`` — every variant/tree with per-rep best cost,
  time and file, plus the averaged best.
* Per-run algorithm JSONs are renamed uniquely (``<instance>_..._repN.json``) so
  no run clobbers another.
...
# ══════════════════════════════════════════════════════════════════════
# Single-config runners (each runs THREE reps and averages)
# ══════════════════════════════════════════════════════════════════════

def run_cluster(ctx, tree_name, dist_path, pop, mutation, mp=None, ms=None):
    """Run one cluster-v3 config (mutation on/off) THREE times and average.

    ``mutation`` toggles SHADE_PAIRWISE_MUTATION; when on, ``mp``/``ms`` set the
    mutation probability/step.  Returns ``{avg, rep1, rep2, rep3}`` where each
    rep is ``{best, time_s, file}``.
    """
    ...
    print(f"\n[{ctx['instance']}/v3-{tag} {tree_name}] avg {avg:.2f} "
          f"(rep1 {reps[1]['best']:.2f} / rep2 {reps[2]['best']:.2f} "
          f"/ rep3 {reps[3]['best']:.2f})")
    return {"avg": avg, "pop": pop, "mp": mp, "ms": ms,
            "rep1": reps[1], "rep2": reps[2], "rep3": reps[3]}


def run_plain(ctx, pop):
    """Run plain SHADE (NOVEL_MUTATION=False) THREE times and average."""
    ...
    avg = float(np.mean([reps[r]["best"] for r in REP_SEEDS]))
    print(f"\n[{ctx['instance']}/plain pop={pop}] avg {avg:.2f} "
          f"(rep1 {reps[1]['best']:.2f} / rep2 {reps[2]['best']:.2f} "
          f"/ rep3 {reps[3]['best']:.2f})")
    return {"avg": avg, "pop": pop,
            "rep1": reps[1], "rep2": reps[2], "rep3": reps[3]}
...
def run_instance(instance, instance_dir):
    """Generate (cached) instance data, then run all 3 variants ×3 reps."""
    print(f"\n{'#'*64}\n# Instance: {instance}\n{'#'*64}")
    inst_out = ROOT / "src" / "outputs" / "instances" / instance
    inst_out.mkdir(parents=True, exist_ok=True)

    baseline_data = ensure_baseline(instance_dir, inst_out)
    activate_instance(instance_dir, baseline_data)
    trees = ensure_distance_trees(instance_dir, baseline_data, inst_out)
    ...
    out_path = inst_out / "best_config_experiments.json"
